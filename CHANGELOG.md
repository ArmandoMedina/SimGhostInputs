...
### Corregido
- **Una curva con dos frenadas reales encadenadas solo anunciaba la primera** (`fantasma/core/corners.py`, `fantasma/viz/pacenotes.py`; [ADR 0033](docs/decisions/0033-frenadas-multiples-por-curva.md), enmienda al ADR 0031): cuando el piloto frena, suelta el pedal, **readministra gas de forma sostenida** (`throttle ≥ 15%` por ≥0.15 s) y vuelve a frenar dentro de la misma curva, el pace note solo sonaba en la primera pisada — la segunda, que a veces es la que realmente entra al ápex, quedaba muda. `select_brake_phase` (el escalar `brake_start`, métrica de coaching/compare) no cambia; se suma `detect_brakings`, que recorre la misma ventana de frenada y publica la lista cronológica de **toda** frenada fuerte real en el campo nuevo `milestones.brake_starts` (solo con ≥2). `viz/pacenotes.py` emite un cue `brake` protegido por cada una — protegido contra protegido, se quedan las dos —, con el countdown de cada una oportunista (puede ceder con `tic_sin_espacio` si choca con el de la frenada anterior de la misma curva). El trail-braking modulado (el pedal nunca baja del piso) sigue sonando como una sola frenada. Validado sobre la vuelta real (BMW M4 GT3, Nordschleife): C03 y C05 emiten 2 frenadas cada una; el escalar `brake_start` y `d_brake_m` quedaron idénticos (`qa_runs/charbel-20260710-frenos-encadenadas/`).
- **`brake_start` sonaba tarde tras un kink: el segmento por curva truncaba el inicio de la frenada** (`fantasma/core/corners.py`, `docs/formato-datos.md`): cada curva analizaba solo su tramo (punto medio con las vecinas), y cuando justo antes había un kink (curva rápida sin frenada) ese punto medio caía en mitad de la zona de frenado y el hito se pegaba al borde del segmento. La detección de frenada ahora usa una ventana propia que arranca **después del ápex de la curva previa** (regla de propiedad: cada curva es dueña de la frenada posterior al ápex de su vecina; un kink no absorbe la frenada de la siguiente). En la telemetría real del Nordschleife: C12 3573→3543 (real 3541), C54 20129→20065 (real 20065), C05 1117→1042. El resto de hitos (ápex, turn-in, gas, coast) sigue usando el segmento por punto medio.
- **`brake_start` señalaba la segunda pisada de una frenada partida** (`fantasma/core/corners.py`): con la frenada dividida en dos bloques (una suelta breve para rotar), `strong[-1]` elegía el último bloque y el cue sonaba ~80 m tarde. Los bloques se agrupan ahora en **fases** (se funden los separados por < `phase_gap_s`=0.5 s con el coche aún desacelerando) y el hito ancla en la **primera muestra de la fase de pico máximo**, donde empieza a cargarse el pedal. En el Nordschleife: C03 801→721, C21 7167→7084. Un blip de trail-braking débil y previo sigue sin adelantar el hito.
- **Los tics de countdown descartados desaparecían sin dejar rastro** (`fantasma/viz/pacenotes.py`): los dos `continue` de la regla de cabida tiraban tics sin registrarlos, y `plan.json` (la auditoría de qué suena y qué no) mentía por omisión. Ahora cada tic descartado sale en `plan["skipped_global"]` con su razón (`antes_de_la_meta` para `d<=0`, `tic_sin_espacio` para el choque de gap) y, en el segundo caso, contra qué evento chocó (`against`: cue, corner_id, distancia). `plan["events"]` sigue siendo la fuente de verdad de lo que se renderiza (los tics descartados no entran ahí).
...
- **`_compile_inno()` salía en silencio si no encontraba `ISCC.exe`** (`tools/build_installer.py`): `python tools/build_installer.py --inno` terminaba con código 0 y sin instalador, y el fallo aparecía más tarde y en otro sitio — el paso "Upload assets" de `release.yml` reventando al buscar un `Setup.exe` que nunca existió. Ahora sale con código 1 en el punto del error, como el resto de los fallos del script.

### Añadido
- **[ADR 0030](docs/decisions/0030-modos-estudio-en-vivo-que-ancla-cada-cue.md) — Modos estudio y en vivo: qué ancla cada cue** (`docs/decisions/0030-...`, `docs/decisions/README.md`, `ROADMAP.md`): registra como decisión de arquitectura la regla de producto que el PO venía explicando entre sesiones sin que quedara asentada (estaba solo en un ítem `[x]` del ROADMAP). Taxonomía de cues por **anclaje**: los de **posición** (frenada, countdown, turn-in, soltar freno, gas, gas a fondo, ápex, coast) se anclan a la distancia de la referencia y viajan en el pack de CrewChief; los de **estado del coche** (cambio de marcha, y futuros por RPM/temperatura/combustible) se anclan al estado del piloto en vivo y **no pueden viajar en un pack de CrewChief** — el formato solo dispara por `distanceRoundTrack`, no por RPM ([ADR 0002](docs/decisions/0002-crewchief-pacenotes.md)). Consecuencia: `gear` queda mudo en el exportable **por diseño**, no por audio pendiente; su sonido es una feature de `fantasma-live`. Enmienda al [ADR 0028](docs/decisions/0028-cues-reencuadre-prioridades-countdown-frecuencias-gear.md): cierra el "slot de sonido" que aquel dejó abierto, señalándolo como trampa para `gear`. Sin cambios de código.
- **`workflow_dispatch` en `release.yml` como vía de rescate** (`.github/workflows/release.yml`): en el evento `release`, GitHub lee el workflow desde el commit al que apunta el **tag**, no desde `master` — así que un bug en el propio workflow obligaba a borrar y recrear tag y release en cada intento (así se quemaron los 3 intentos de `v2.3.0`). Un `workflow_dispatch` se lee siempre desde la rama por defecto: ahora se puede re-disparar el build contra un release ya publicado pasándole el tag como input, sin tocar el tag. El código se sigue compilando desde el tag (`checkout` con `ref`), no desde `master`, para que el instalador corresponda al release.
- **Workflow `installer-smoke`** (`.github/workflows/installer.yml`): ensaya el empaquetado completo (`nicegui-pack` + Inno Setup) en cada PR que toque `main_gui.py`, `pyproject.toml`, `tools/build_installer.py`, `tools/installer.iss` o cualquiera de los dos workflows, y publica el `Setup.exe` como artefacto descargable del run (14 días) para poder probar la instalación **antes** de cortar el tag. Ataca la causa raíz de los 3 fallos de `v2.3.0`: un pipeline que solo corría al publicar un release, y que por tanto llevaba desde el ADR 0022 sin ejercitarse ni una vez.
...
- **Manejo de errores ciego en los 3 hooks de sesión (ALTO-04, `qa_runs/2026-07-03-auditoria-integral/fase3-hooks.md`)** (`.claude/hooks/review-stop.ps1`, `mariana-stop.ps1`, `escribano-stop.ps1`): se quita el `$ErrorActionPreference = 'SilentlyContinue'` global de los 3 scripts, que silenciaba cualquier fallo real de `git status`/`git diff` (git no disponible, repo corrupto, permisos) y lo dejaba pasar como "sin cambios que revisar" — un falso negativo del hook que vigila a los demás agentes. Ahora cada llamada real a git se revisa por `$LASTEXITCODE`; si falla de verdad, el hook **avisa** (`additionalContext`, sin `decision: block` — sigue sin bloquear) en vez de callar. Verificado a mano con un repo de prueba (índice de git corrompido a propósito): antes, los 3 scripts salían en silencio con exit 0 y sin JSON; ahora los 3 emiten el aviso. Alcance acotado a ALTO-04: el resto de hallazgos de `fase3-hooks.md` (CRITICO-01/02, ALTO-01/02/03, MEDIO-01/02) queda fuera a propósito — ya evaluados y aceptados por disciplina de proceso en el [ADR 0019](docs/decisions/0019-adopcion-homologacion-starter-v0.5.0.md).
- **Las notas de VOZ no pasaban por el plan anti-saturación** (`fantasma/viz/pacenotes.py::build_voice_pack`; [ADR 0024](docs/decisions/0024-sincronia-pace-notes.md), enmienda "notas de voz"): `build_voice_pack` recalculaba su propio anticipo (200 m fijos) y su propio descarte de `distance<=0`, sin ningún gap mínimo entre curvas — dos frenadas cercanas podían generar narraciones de ~7.5 s que se encimaban. Ahora reutiliza `_resolve_min_gap`, la MISMA resolución de gap global que ya usa `plan_tone_events` (extraída a función de módulo para compartirla sin duplicar la lógica), y el anticipo se deriva de la velocidad de llegada a la frenada (`_voice_lead_m`, `DEFAULT_VOICE_LEAD_S=4.0` s, acotado a [60, 400] m) en vez de un offset fijo — fallback a los 200 m históricos si el milestone no trae `v` (no-regresión exacta para fixtures/corners viejos sin canal de velocidad). `build_pack(mode="voice"|"both")` reenvía los nuevos `min_gap_m`/`voice_lead_s` a `build_voice_pack` igual que ya hacía con los kwargs de `build_tone_pack` (antes se perdían en silencio, hallazgo de `/code-review`). Comportamiento observable: con telemetría real (que sí trae `v`), las distancias narradas cambian levemente respecto al fijo de 200 m; las entradas de `metadata.json` quedan ordenadas por distancia (antes, por orden de mayor `time_lost`) — mismo criterio que ya usa `build_tone_pack`, sin efecto en la reproducción (`render_pace_notes_track`/`build_cue_ass` no dependen del orden). El número de curvas narradas por defecto (top-N) no cambia. 11 tests nuevos en `tests/viz/test_pacenotes.py`.
- **El job de render del Paso 3 vivía en variable local, no en `state`** (`fantasma/ui/ng_step3.py`, `fantasma/ui/ng_state.py`; ROADMAP): al navegar fuera del Paso 3 durante un render activo se cancelaba solo el timer de polling, pero el job seguía corriendo en background; volver al Paso 3 recreaba el `job_holder` local vacío, así que el guard de "Generar overlay" no veía el render en curso y podía arrancar un **segundo render concurrente sobre el mismo `outdir`** (riesgo de corromper el `.webm` de salida). Corregido guardando el `RenderJob` en la nueva propiedad `state.active_overlay_job`, respaldada por `app.storage.tab` (memoria de la pestaña, sobrevive también a un refresh de página) en vez de `app.storage.user` (persistido a JSON en disco, donde un objeto con `threading.Event` rompería el respaldo). Al reentrar al Paso 3 con un render activo, la app reengancha la barra de progreso sobre el job existente en vez de ofrecer "Generar overlay" de nuevo. Pre-existente; destapado por el Reviewer en `feat/pacenotes-ui`.
...
### Añadido
- **Lockfile de dependencias** (`requirements-lock.txt`, extra `dev` de `pyproject.toml`; [ADR 0029](docs/decisions/0029-lockfile-pip-compile.md)): versiones exactas resueltas con `pip-compile` (`pip-tools`) desde los extras `full`, `test` y `dev` de `pyproject.toml` — auditable contra CVEs, builds reproducibles. Se suma `pip-tools>=7,<8` al extra `dev`. Regenerable a mano en el proceso de release (`CONTRIBUTING.md` §3); no reemplaza el flujo normal de instalación (`pip install -e ".[full]"`). Cierra la deuda técnica Media del `ROADMAP.md` (auditoría 2026-07-03, `fase1-seguridad.md`).
- **Test E2E: el mux del Paso 5 ejerce `app.storage.user` real en un hilo `run.io_bound`** (`tests/ui/visual/test_e2e_step5_mux_storage_context.py`; cierra el ítem de ROADMAP "Ningún test unitario cubre lectura de `app.storage.user`..."): el mock plano `_StateWithRowsNoDrv` (`tests/ui/test_ng_step5.py`) evade por completo el proxy real de NiceGUI sobre `app.storage.user`, que solo lanza `RuntimeError` fuera de contexto UI — exactamente donde corre un hilo de `run.io_bound`. Por eso el bug real de `_do_mux` (leer `state.drv_name` dentro del hilo en vez de capturarlo antes; corregido en #30) pasó la suite en verde y solo lo cazó el E2E Playwright manual. El test nuevo clickea "Aplicar sonido" contra un server NiceGUI real (mismo patrón de `tests/ui/visual/conftest.py`), con `state.drv_lap`/`state.drv_name` poblados subiendo dos CSVs sintéticos mínimos (sin telemetría real), y verifica que el error mostrado nunca sea el de `app.storage.user`/contexto UI. Verificado reintroduciendo el bug a mano: el test lo cazó.
- **Cue `gear` (cambio de marcha), acotado a subtítulo** (`fantasma/core/corners.py::detect_gear_shifts`, `fantasma/viz/pacenotes.py`, `fantasma/cli.py`, `fantasma/ui/`; [ADR 0028](docs/decisions/0028-cues-reencuadre-prioridades-countdown-frecuencias-gear.md), enmienda al ADR 0027): el slot `gear` que el 0027 dejó reservado y sin implementar queda cerrado. `detect_gear_shifts(lap, min_hold_s=0.15)` recorre la vuelta de **referencia** completa (lap-wide, no por curva) y devuelve `[{"distance", "gear_from", "gear_to"}, ...]` con debounce anti-blip. Wireado end-to-end: `fantasma detect` escribe `gear_shifts` junto a `corners` en `corners_detected.json`, `fantasma pacenotes` lo consume, y la UI NiceGUI lo detecta en los Pasos 1/2/3 y lo reenvía al Paso 5. El cue entra apagado por defecto, con subtítulo `"cambio a Nª"` en magenta — **sin audio todavía** (nuevo campo `sound: false` en su config, ver siguiente entrada).
- **Campo `sound` en el esquema de configuración de cues** (`DEFAULT_CONFIG` de `fantasma/viz/pacenotes.py`, `fantasma/viz/cue_profiles.py`; [ADR 0028](docs/decisions/0028-cues-reencuadre-prioridades-countdown-frecuencias-gear.md)): separa "qué suena" de "qué se subtitula" — un cue con `sound: false` sigue participando en la resolución de cabida/prioridad y en el subtítulo quemado, pero `build_tone_pack` no le sintetiza WAV. Default `True` para todo el catálogo salvo `gear`. Los perfiles JSON de terceros pueden forzar `sound: true` en cualquier cue.

### Cambiado
- **Reencuadre de prioridades, countdown y frecuencias de cues** (`fantasma/viz/pacenotes.py`; [ADR 0028](docs/decisions/0028-cues-reencuadre-prioridades-countdown-frecuencias-gear.md), enmienda a los ADR 0025 y 0027), a partir de feedback de oído del PO sobre la cinta `2_estudio_coast_ADR0027.mp4`: **prioridades** reordenadas (`throttle_on=95`, `full_throttle=90`, `turn_in=70` en su propio escalón — antes competía en desventaja contra ambos —, `brake_countdown=50`, `brake_release=45`, `coast=20` como oportunistas; `brake=100` sigue protegido). **Countdown de frenada** con gap uniforme: `DEFAULT_COUNTDOWN_GAP_S = 0.75` (0.75 s parejos entre los 3 sonidos) reemplaza `DEFAULT_COUNTDOWN_S = 3.5` (fracciones de un anticipo total, ~1.75 s por gap — el PO lo reportó "muy lento y a veces no completa los 3 sonidos"); clamps de `_countdown_lead_m` bajan de `[60, 350]` a `[30, 250]` m. **Frecuencias** reajustadas para eliminar colisiones perceptivas reportadas (`brake_countdown: 800`, `brake_release: 820`, `turn_in: 500` — ya no comparte 660 Hz con el primer tic del countdown —, `full_throttle: 320`, `throttle_on: 250`, `gas_100: 190`, `coast: 160`, `gear: 650`). Leyenda del Paso 5 actualizada.
...
### Corregido
- **Los botones de acción del wizard se veían "apagados", como deshabilitados aunque estuvieran activos** (`fantasma/ui/ng_app.py`): los CTA `.btn-primary`/`.btn-featured` se construyen con `.props("flat")`, y el `flat` de Quasar inyecta `text-primary` (`color: var(--q-primary) !important` = `#5898d4`) que **gana** a nuestro `.btn-primary { color: white !important }` por orden de carga (misma especificidad, la hoja de Quasar va después) → **texto azul sobre fondo azul** (~1.4:1, ilegible); además habilitado (opacity 1) casi no se distinguía del deshabilitado. Corregido subiendo la especificidad del color a `.q-btn.btn-primary`/`.q-btn.btn-featured` (y su `.q-btn__content`), que gana sin depender del orden → **texto blanco sobre azul sólido** en activo y `opacity 0.4` solo en deshabilitado. Reportado por el PO; causa raíz medida con `getComputedStyle` (QA Mariana, evidencia en `qa_runs/mariana-20260706-cinta-overlay-sync/`).
- **Los inputs de ruta del Paso 4 truncaban la ruta** (`fantasma/ui/ng_app.py`): la columna izquierda es `align-items: flex-start` y `.panel` no fijaba `width`, así que los paneles ① (Archivos) y ③ (Carpeta de salida) se encogían a su contenido (input ~159 px, `C:\Users\jose_\Download…`) mientras ② y ④ se veían anchos solo por su texto largo. Corregido con `.panel { width: 100% }` — los paneles llenan la columna y los inputs `flex-1` se estiran; de paso deja de truncarse el label flotante del Paso 4.
