...
### Añadido
- **Subtítulos de cues en el video de estudio** ([ADR 0027](docs/decisions/0027-subtitulos-de-cues-en-video.md)): el Paso 4 gana el checkbox «Añadir subtítulos que nombran cada sonido» (dentro de la sección de Pace Notes). Al activarlo, `compose_video(burn_cue_subs=True)` genera un `.ass` desde el pack (`build_cue_ass` en `pacenotes.py`: un rótulo por cue con color por tipo + nombre de curva, más una leyenda) y lo quema en el **mismo encode** que superpone el HUD y mezcla los tonos. El texto se sincroniza con el mismo `_dist_to_time` que el audio, así rótulo y tono caen juntos; los colores viven en una fuente única (`CUE_SUB_COLORS`). Resuelve el "ando bien perdido con qué significa cada sonido" del PO y desbloquea la validación auditiva del [ADR 0026]. 5 tests nuevos (`build_cue_ass` + wiring de `burn_cue_subs`); tabla de colores en `docs/hud-reference.md`.
