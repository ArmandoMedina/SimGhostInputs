...
6. **`.props("flat")` en un CTA con color propio: fijar el color con `.q-btn.<clase>`, no con `.<clase>`.** Casi todos los botones del wizard se construyen `.classes("btn-primary …").props("flat")`. El `flat` de Quasar inyecta `text-primary` (`color: var(--q-primary) !important`) que, a **igual especificidad** que `.btn-primary { color: white !important }`, **gana por orden de carga** (la hoja de Quasar se inyecta después de nuestro `add_head_html`) → texto azul sobre fondo azul, ilegible, el botón "se ve apagado" aunque esté activo (y no se distingue del deshabilitado). Regla: el color de texto de un `.btn-primary`/`.btn-featured` con `flat` **debe** fijarse con selector `.q-btn.btn-primary` (+ su `.q-btn__content [span]`), especificidad `0,2,0`, que le gana a Quasar (`0,1,0`) sin depender del orden. La misma trampa aplica a la opacidad del estado deshabilitado (Quasar la deja en 0.7; para forzar 0.4 hay que usar `.q-btn.btn-primary:disabled`). Detectado midiendo `getComputedStyle` (QA Mariana 2026-07-06).
