...
### 3.8 Pace Notes desde la UI

| # | Caso | Veredicto | Notas |
| :-- | :-- | :-- | :-- |
| C36 | Traer un video con overlay ya hecho y solo añadir pace notes (flujo "Solo Pace Notes" desde la UI) | ✅ | El usuario elige la tarjeta 🔔 "Solo Pace Notes" en el Paso 0; importa ref+piloto en el Paso 1, corre el análisis en el Paso 2, llega al Paso 5. En el panel ① genera el pack WAV; en el panel ② indica el video ya compuesto y la carpeta del pack, y el sistema aplica el audio sin re-encodear el video (ffmpeg `-c:v copy`). Complementario a C34 (headless CLI): mismo resultado, distinta interfaz. Restricción: exige dos vueltas (ref+piloto) porque el pack prioriza curvas por tiempo perdido (`compare()`); no se puede generar de una sola vuelta ni directamente desde el video. |
