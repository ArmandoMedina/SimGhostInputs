...
4. **Frenada real (fases)**: la frenada se busca en una ventana que arranca **justo después del ápex de la curva previa** (no en el punto medio), con el mismo tope de 450 m hacia atrás. Los puntos con `brake` > `brake_on` se agrupan en **bloques**, y los bloques consecutivos se funden en **fases** cuando el hueco temporal entre ellos es menor que `phase_gap_s` (0.5 s por defecto) **y** el coche sigue desacelerando en el hueco (una suelta breve del pedal para rotar, no una acción distinta); un toque de freno aislado que quedó lejos, o tras el cual el coche re-aceleró, queda en su propia fase. El piso de intensidad `brake_strong` (50% por defecto — el freno de verdad, el que evita pasarse de curva) es un **filtro**: se **descartan** las fases que no lo alcanzan. Entre las fases que quedan, `brake_start` ancla en la **primera muestra de la de pico máximo**; ante empate de pico, la **más tardía** (la que entra al ápex); si **ninguna** fase alcanza el piso, la **última fase cronológica**. El porqué: el cue marca dónde empezar a **cargar el pedal hacia el máximo freno** aprovechando la transferencia de peso, así que gana la pisada de mayor intensidad, no la última. Un blip débil y previo del trail braking queda por debajo del piso y **no adelanta** el hito.
   ...
   - **Frenadas múltiples reales, para el audio** ([ADR 0033](decisions/0033-frenadas-multiples-por-curva.md)): `select_brake_phase` (arriba) resuelve **una sola** pregunta — el punto de frenada comparable, `brake_start`, que no cambia. Sobre la misma ventana, `detect_brakings` (`fantasma/core/corners.py`) responde una pregunta distinta — **qué frenadas deben sonar** — y recorre los mismos bloques de freno agrupándolos igual que las fases, pero separa dos frenadas cuando entre ellas el piloto **readministra gas de forma sostenida** (`throttle ≥ THROTTLE_REAPPLY`, 15% por defecto, durante ≥ `THROTTLE_REAPPLY_WINDOW_S`, 0.15 s): eso distingue "volvió a acelerar y frenó otra vez" de "soltó un instante para rotar" (trail-braking), que sigue fundiéndose en un solo bloque. Devuelve la lista cronológica de toda frenada fuerte (pico ≥ `brake_strong`); `extract_milestones` la publica en `milestones.brake_starts` **solo cuando hay ≥2**. El porqué de separar las dos preguntas (y por qué `brake_start` no se toca) es del ADR 0033, no se repite aquí.
  ...
  "corners": [
    {
      ...
      "milestones": {
        ...
        // brake_starts: opcional, SOLO si la curva tiene >=2 frenadas fuertes
        // reales (ADR 0033) — lista cronológica, misma forma que brake_start:
        // "brake_starts": [
        //   {"d": 7167, "t": 133.9, "v": 142, "gear": 2, "brake_pct": 100},
        //   {"d": 7244, "t": 135.9, "v": 96,  "gear": 1, "brake_pct": 90}
        // ]
...
**`brake_start` (escalar) vs. `brake_starts` (lista) — qué campo usar** ([ADR 0033](decisions/0033-frenadas-multiples-por-curva.md)): `brake_start` es la frenada de **pico máximo** de la curva (punto 4 arriba) y es la métrica de coaching/compare — `compare()` y todo consumidor previo siguen leyendo solo este campo, sin cambio. `brake_starts` es la lista de **todas** las frenadas fuertes reales que deben sonar en el audio de pace notes; solo aparece cuando hay ≥2, y su único consumidor es `fantasma/viz/pacenotes.py`. No son redundantes: responden preguntas distintas (qué se compara vs. qué suena).
