...
- [ ] **Ningún test unitario cubre lectura de `app.storage.user` en hilos `run.io_bound`** — el harness de `nicegui.testing` mockea `state` como objeto plano (`_StateWithRowsNoDrv` en `tests/ui/test_ng_step5.py`), así que `AppState`→`app.storage.user` nunca se ejerce y esta clase de bug pasa la suite en verde (el mux del Paso 5 se envió roto por esto; corregido en #30, solo lo cazó el E2E Playwright). Opciones: una guarda que ejercite el proxy real en un hilo, o llevar el mux al E2E de `tests/ui/visual/`. _Prioridad: Media._
...
- [ ] **`throttle_on_window`/`full_throttle` cuentan en muestras fijas, no normalizadas por tasa de muestreo** (`fantasma/core/corners.py`) — el umbral de sostenibilidad (N muestras) queda mal calibrado en telemetría con frecuencia distinta de 50 Hz. Ver [ADR 0027](docs/decisions/0027-cues-catalogo-configurable-perfiles-coast-subtitulos.md). _Prioridad: Media._
...
- [ ] **`detect_gear_shifts` reimplementa el patrón de "ventana sostenida"** (`fantasma/core/corners.py`) en vez de reusar el que ya usan `throttle_on`/`full_throttle` en el mismo archivo — resuelve un problema distinto (transición discreta entre marchas con debounce de confirmación, no un umbral continuo sostenido), pero deja dos patrones de "sostenimiento" separados en el módulo. Deliberado, para no ampliar el diff del release ([ADR 0028](docs/decisions/0028-cues-reencuadre-prioridades-countdown-frecuencias-gear.md)), señalado por un `/code-review` de alto esfuerzo. _Prioridad: Media._
