...
- [ ] **Consumidores de `segment_m` que asumen contención, pero `brake_start` puede caer fuera** — el [ADR 0031](docs/decisions/0031-propiedad-de-la-frenada-y-contrato-de-segment-m.md) fija que `segment_m` es una banda de vecindad, no un contrato de contención: los hitos mandan (medido: 7 de 55 curvas violan la invariante, desfase máx. 64 m, `qa_runs/2026-07-09-revalidacion-corners/`). Dos consumidores siguen derivando su ventana de `segment_m` en vez del hito: (1) `fantasma/viz/overlay.py:115` (`_corner_at`) etiqueta en el HUD la curva ANTERIOR mientras el pace note ya pertenece a la SIGUIENTE durante la frenada extendida; (2) `fantasma/viz/charts.py:53-54` (`plot_corner`) recorta con padding fijo de 120 m — hoy no trunca porque el desfase máx. es 64 m, pero es defensa incidental, no regla. El arreglo es derivar la ventana del `brake_start`, como ya hace `charts.py:374-377` (`min(ref_bd, drv_bd) - 80`). `compare.py:322` (mismo problema, `d_brake_m` asimétrico → bandera `"frenada"` espuria) se arregla en paralelo con un helper compartido. _Prioridad: Media._
...
- [x] **Detección de cambio de marcha (cue `gear`)** — implementado **solo-subtítulo**: `detect_gear_shifts` (`fantasma/core/corners.py`) wireado end-to-end (CLI + UI), subtítulo `"cambio a Nª"` en magenta. Sin sonido a propósito (`sound=False`). Ver [ADR 0028](docs/decisions/0028-cues-reencuadre-prioridades-countdown-frecuencias-gear.md) (enmienda al ADR 0027).
  - **Regla de diseño del PO (modo estudio vs. en vivo): registrada en el [ADR 0030](docs/decisions/0030-modos-estudio-en-vivo-que-ancla-cada-cue.md).** El cambio de marcha es un cue de **estado del motor**, no de posición: en estudio se subtitula desde la referencia, pero su sonido no debe viajar en el pack de CrewChief (que solo dispara por metro de pista, no por RPM) — es una feature de `fantasma-live`. Por eso `gear` se queda mudo en el exportable **por diseño, no por audio pendiente**: el ADR 0030 cierra el "slot de sonido" que el ADR 0028 dejó abierto.
...
- [ ] **Nadie ha verificado que CrewChief acepte una entrada de `metadata.json` sin audio** — un cue mudo (`sound=False`, hoy solo `gear`) emite su entrada con `fileNames: []`/`recordingNames: []` (`fantasma/viz/pacenotes.py:1061-1082`, declarado en `docs/formato-datos.md:113`), pero el spec original del pace note ([ADR 0002](docs/decisions/0002-crewchief-pacenotes.md)) siempre trae un WAV (`fileNames: ["<dist>_0.wav"]`, `docs/decisions/0002-crewchief-pacenotes.md:62-63`). Si el parser de CrewChief asume que la lista nunca está vacía, un pack con cues mudos podría reventar o ignorarse en pista. Requiere QA con CrewChief real antes de exportar un pack que mezcle cues sonoros y mudos. _Prioridad: Media._
  - **Mecanismo concreto (auditoría del código fuente de CrewChief, 2026-07-09):** `getRandomRecordingName()` hace `recordingNames[Utilities.random.Next(recordingNames.Count)]`; con `Count == 0`, `Next(0)` devuelve `0` → `recordingNames[0]` lanza `ArgumentOutOfRangeException`. Nuestros cues mudos (`gear`) emiten `fileNames`/`recordingNames: []`, que es justo `Count == 0`. **NO está verificado al 100 %:** el auditor no pudo trazar que la ruta de reproducción filtre las entradas sin audio ANTES de esa llamada, así que no se sabe si el crash se alcanza de verdad; solo se manifestaría en pista, no en la generación del pack. Arreglo propuesto si se confirma: exportar un **WAV silencioso** para los cues mudos en vez de lista vacía (los subtitula igual y evita el `Count == 0`). Ver riesgo conocido en [ADR 0030](docs/decisions/0030-modos-estudio-en-vivo-que-ancla-cada-cue.md). _Prioridad: Media._
  - **Opción C (frontera estudio/vivo, Fase 5):** separar el `metadata.json` del PACK de CrewChief del metadata del SUBTÍTULO, para que los cues mudos/anclados-a-RPM (`gear`) NO viajen en el pack de CrewChief — hoy se embarcan como WAV silencioso (parche de #9); la versión limpia es no embarcarlos. _Prioridad: Baja (diferida)._
- [ ] **Doc-drift: estado del ADR 0002 discrepa entre el índice y el propio ADR** — `docs/decisions/README.md:14` lista el [ADR 0002](docs/decisions/0002-crewchief-pacenotes.md) como "Propuesta (diferida post-v1.0)", mientras que el encabezado del propio ADR dice "Aceptada · implementada (v2.0, Unreleased)" (`docs/decisions/0002-crewchief-pacenotes.md:4`). El pace note está implementado en este ciclo, así que el índice es el que quedó atrás. Sincronizar el estado en el índice (tarea del Escribano/Armando). _Prioridad: Baja._
- [ ] **El canal `steering` mezcla `Steering Pos` (%) y `STEERANGLE` (grados) sin normalizar, y el umbral se llama "deg"** (`fantasma/importers/motec_csv.py:23,34`; umbral `turn_in_deg=8` en `fantasma/core/corners.py:187`) — ambas columnas de MoTeC caen en el mismo canal `steering` sin conversión de unidad, y el motor consume ese canal, no `STEERANGLE`. Medido sobre material real: la fila de unidades de **ambos** CSV declara `Steering Pos` en `%`, y `|steering|` da absmax 50 % (REF) / 74 % (PILOTO), mediana ~2.6 %, p90 ~15 % — no son grados de volante (un GT3 de ±240° marcaría 100–200° en una horquilla). Por tanto `turn_in_deg=8` compara contra el **8 % del recorrido del volante**, no contra 8° (`qa_runs/2026-07-09-turn-in-reglas/diagnostico.md`, sección "Unidad del canal `steering`"). **Matiz honesto:** el análisis de turn-in NO queda invalidado — 8 % cae entre mediana y p90, es un umbral funcional de "volante comprometido", y la comparación entre reglas es agnóstica a la unidad; lo que falla es la **etiqueta**. El bug muerde el día que un importador emita `STEERANGLE`: metería grados en un canal calibrado en % sin que nada avise (relevante para los importadores `.ibt`/rF2 del roadmap). Arreglo: normalizar por unidad al importar (o separar canal/etiqueta) y renombrar el umbral. Toca el modelo de datos → coordinar con `docs/formato-datos.md` (SSOT) y Charbel. _Prioridad: Media._
- [ ] **`brake_strong=50` es demasiado alto para la frenada floja del piloto → banderas "frenada" falsas por tarde** (`fantasma/core/corners.py:42`, filtro de `select_brake_phase`) — medido adversarialmente sobre material real: en C05/C32/C48 el piloto frena con picos 39/37/41 %, nunca ≥50 %, así que el piso absoluto no ancla ninguna fase fuerte y la selección cae a una re-pisada tardía → bandera "frenada" **falsa por tarde** (`qa_runs/2026-07-09-fusion-por-deceleracion/diagnostico.md`, Tablas 2 y 5). **La causa raíz NO es la regla de fusión** (se midieron 4 reglas de selección de fase R0/R1/R2/R2'; R2 corrige 2 de 3 "por suerte estructural" al fundir hacia el primer bloque, pero **no cura C48**, donde ancla en un roce de 11 %). La palanca correcta —**aún sin medir**— es un piso RELATIVO al pico de la ventana en vez del absoluto 50. **Hace falta un ciclo que MIDA el piso relativo** (¿fracción del pico?, ¿mínimo absoluto?) sobre varias vueltas ANTES de tocar `corners.py`. _Prioridad: Media._
- [ ] **`turn_in`: dos defectos latentes confirmados que hoy no muerden; la regla vigente se mantiene** (`fantasma/core/corners.py:295-316`) — la regla actual (primer cruce ascendente del umbral desde `brake_start` hasta el ápex) tiene dos defectos reproducidos por ejecución sobre material real (`qa_runs/2026-07-09-turn-in-reglas/diagnostico.md`, Tabla 2 + las dos búsquedas dirigidas): **(A)** si el volante ya supera el umbral en `brake_start`, `turn_in` queda en `None`; **(B)** el barrido puede leer un cruce antes de `segment_m[0]`. En la vuelta de Nordschleife **ninguno muerde**: las 3 instancias de (A) son C09/C11/C44 —kinks sin frenada cuyo turn-in la regla debe suprimir (T0/T3 los resucitarían como cues fantasma audibles)—; las 2 de (B) son C14/C15 (6–7 m), turn-ins legítimos propios durante trail-braking. Se midieron **5 reglas alternativas** (T0/T1/T3/T4/T5) y **ninguna mejora a la vigente** (T0/T3 resucitan fantasmas; T1 pierde C14; T4/T5 atrasan C15/C50/C52). **Decisión: la regla se queda.** El defecto (B) no es un bug: es el contrato de [ADR 0031](docs/decisions/0031-propiedad-de-la-frenada-y-contrato-de-segment-m.md) ("los hitos mandan, `segment_m` no contiene") y el propio código lo documenta (`corners.py:299-302`). **Condición de reapertura:** solo si aparece material de otra pista donde (A) borre un `turn_in` en una curva **con frenada** (no un kink fantasma). _Prioridad: Baja._
