...
- [ ] **Canonizar la unidad del volante en el importer (o hacer `turn_in` relativo al pico de volante de la curva, libre de unidad) — DIFERIDO, bloqueado por falta de material.** MEDIDO e inerte en el par real (`qa_runs/charbel-20260709-deudas/`): REF y piloto exportan **ambos** `Steering Pos` en %, absmax ~36; `turn_in_deg=8` cae en ~25-33 % del rango realmente usado (giro comprometido, umbral sano), 0 de 55 curvas mal ubicadas. Es un **landmine LATENTE de código**: `fantasma/importers/motec_csv.py:34` mapea `STEERANGLE` (grados) al mismo canal `steering` que `Steering Pos` (%) (`:23`), y `turn_in_deg` no lleva unidad, así que explotaría con una vuelta exportada en grados (rango ~±cientos), donde `8` significaría 8 grados y el turn_in casi nunca dispararía (relevante para los importadores `.ibt`/rF2 del roadmap). El fix requiere una vuelta real con `STEERANGLE` para QA; sin ese material no hay veredicto (regla de artefacto). No tocar a ciegas; al abordarlo, coordinar con `docs/formato-datos.md` (SSOT) y Charbel. _Prioridad: Media (diferida)._
- [x] **`brake_strong=50` — MEDIDO: mantener en 50, no bajar. CERRADO por medición, sin acción de código** (`fantasma/core/corners.py:42`, filtro de `select_brake_phase`; evidencia `qa_runs/charbel-20260709-deudas/`). Barrido {50,45,40,35} sobre la referencia: **0 `brake_start` se mueven** (las curvas de freno son monofásicas; fallback "última cronológica" y "pico máximo" coinciden, el piso es irrelevante ahí). De 15 banderas `frenada` piloto-vs-ref, **13 son legítimas** (diferencias reales de punto de frenada, hotlap vs carrera +20 s) y 2 marginales (C08 pico 48, C10 pico 26; la ref ancla sobre un lift ligero, y nacen de `brake_on`, no del piso). Ninguna se quita bajando el piso. Palanca si C08 y C10 molestaran: `brake_on` o un piso de pico mínimo en kinks, **no** `brake_strong`. Reabrir solo si aparecen curvas multifásicas donde el piso sí mueva el ancla.
- [x] **`turn_in`: dos defectos latentes, MEDIDOS e inertes en el par real — CERRADO, sin acción propia** (`fantasma/core/corners.py:295-316`; evidencia `qa_runs/charbel-20260709-deudas/`). (1) Ceguera de unidad de `turn_in_deg`: es el mismo landmine de la viñeta de steering de arriba (dormido con datos en %); su fix se rastrea allí, no aquí. (2) `turn_in` puede caer antes de `segment_m[0]` (C14: 4845 vs seg0 4850, −5 m; C15: 4976 vs 4982, −6 m) porque el scan arranca en `brake_start`; esto es **por diseño** (`docs/formato-datos.md` §5: el turn_in NO se acota a `segment_m[0]` a propósito, para no perderlo cuando la frenada precede al segmento) y el contrato de [ADR 0031](docs/decisions/0031-propiedad-de-la-frenada-y-contrato-de-segment-m.md) ya obliga a los consumidores a no asumir que un hito cae dentro de `segment_m`. Inocuo (5-6 m). Un tercer modo candidato (re-cruce del volante residual) fue **refutado: 0 casos** (la regla de cruce ascendente lo cierra bien). **Reapertura:** solo si aparece material de otra pista donde el defecto de unidad borre un `turn_in` en una curva **con frenada** (no un kink fantasma).
