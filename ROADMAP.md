...
- [ ] **Verificar contra un `Agent` real el campo `tool_input.isolation` que consume `agent-concurrency-gate.ps1`** — el hook (enmienda ADR 0019, 2026-07-09) solo se probó con stdin sintético; si el nombre/forma real del campo no coincide, el `try/catch` interno hace fail-open (`exit 0`) sin ningún aviso y el tope de 3 queda inerte sin que nadie lo note. Cerrar en cuanto se retome el primer worktree-agent real tras el reset del cupo de cuenta. _Prioridad: Media._
