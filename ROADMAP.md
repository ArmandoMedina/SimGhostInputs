...
- [ ] **La versión del footer está hardcodeada** (`ng_app.py`, `version-badge`) — el badge del sidebar («v2.1») es un literal desacoplado de `pyproject.toml`; hay que bumpearlo a mano en cada release (ya pasó: se quedó en «v2.0» tras el bump a 2.1.0). Unificar con una única fuente de versión robusta en el exe congelado (evitar `version("fantasma-inputs")` que en PyInstaller puede resolver mal). _Prioridad: Baja._
