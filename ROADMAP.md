...
- [ ] **La versión del footer está hardcodeada** (`ng_app.py`, `version-badge`) — el badge del sidebar («v2.1») es un literal desacoplado de `pyproject.toml`; hay que bumpearlo a mano, pero el gate §8 lo separa del PR de release (un cambio en `ui/` exige su doc dueño), así que quedó en «v2.1» tras 2.2.0. Unificar con una única fuente de versión robusta en el exe congelado (evitar `version("fantasma-inputs")` que en PyInstaller puede resolver mal). Si se bumpea a mano, hacerlo en un PR `fix(ui)` propio con su doc, no en el de release. _Prioridad: Baja._
