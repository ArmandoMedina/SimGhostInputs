...
- **Countdown oportunista, por cabida y no por severidad, con la regla «solo cede lo que puede ceder»** ([ADR 0026](../../docs/decisions/0026-cues-frenada-universal-countdown-oportunista.md) y [ADR 0032](../../docs/decisions/0032-regla-de-cabida-del-countdown-solo-cede-lo-que-puede-ceder.md), enmiendas al 0025): en cada frenada protegida con `lead_m`, se intentan 2 tics de aviso (`brake_tic`, `step` 0 y 1, en `brake_d - lead_m` y `brake_d - lead_m/2`). Un tic se coloca salvo que su hueco (`< min_gap_m`) lo ocupe un sonido **no cedible** — una **frenada protegida** o un **tic del countdown de OTRA curva** —, y solo entonces es el **tic** el que cae (razón `tic_sin_espacio`). Un cue **no protegido** de otra curva (`turn_in`, `throttle_on`, `full_throttle`, `brake_release`, `coast`) **no bloquea el tic: cede su hueco** y sale del plan sonoro con razón `cedio_al_countdown` (registrando el tic contra el que chocó — sin descartes silenciosos). **Invariante de seguridad (no negociable): un tic NUNCA desplaza una frenada protegida** — la frenada es, por construcción, uno de los dos sonidos no cedibles que bloquean al tic; si chocan, cede el tic, nunca la frenada. El porqué de la regla, su medición y trade-offs viven en el [ADR 0032](../../docs/decisions/0032-regla-de-cabida-del-countdown-solo-cede-lo-que-puede-ceder.md), no se re-narran aquí. `lead_m` sigue calculándose **por tiempo**: `countdown_s` segundos a la velocidad de llegada, acotado a [30, 250] m (`_countdown_lead_m`, ADR 0024); sin `v`, fallback fijo `countdown_m`. En curvas muy densas (con dos frenadas ajenas pegadas) los tics pueden no caber y la curva queda solo con su tono de frenada — no hay gate de severidad (`time_lost`/`braking_issue`) que decida si el countdown aplica.
...
- Dado un hito de frenada con velocidad `v`, cuando se planea el countdown, entonces el anticipo (`lead_m`) equivale a `countdown_s` segundos a esa velocidad (acotado a [30, 250] m); sin `v`, se usa el fallback fijo `countdown_m`.
- Dado un tic de countdown (`brake_tic`) y un cue **no protegido** (turn-in, gas, gas a fondo, soltar freno, coast) en su hueco a menos de `min_gap_m`, cuando se planea, entonces el cue no protegido **cede** (queda en `skipped` con razón `cedio_al_countdown` y el tic contra el que chocó) y el tic suena ([ADR 0032](../../docs/decisions/0032-regla-de-cabida-del-countdown-solo-cede-lo-que-puede-ceder.md)).
- Dado un tic de countdown cuyo hueco (`< min_gap_m`) está ocupado por una **frenada protegida** o por un **tic de otra curva**, o que caiga en `d ≤ 0`, cuando se planea, entonces es el **tic** el que se descarta (razón `tic_sin_espacio`) y esos sonidos no cedibles se conservan — la frenada protegida nunca se pierde ni se mueve.
