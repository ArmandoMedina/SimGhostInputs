...
## Reglas de negocio
- Si `state.rows` o `state.corners` no están disponibles, el Paso 5 muestra un aviso y un botón de vuelta al Paso 2; no se renderiza el panel de generación.
...
- Dado que `state.rows` o `state.corners` es None, cuando se renderiza el Paso 5, entonces se muestra el aviso «Primero corre el Análisis (Paso 2)» y el botón de vuelta al Paso 2, y no se renderiza el panel de generación.
