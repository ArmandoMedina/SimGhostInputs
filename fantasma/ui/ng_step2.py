...
async def render(state, navigate):
    ...
    with ui.row().classes("gap-4 mb-4 w-full"):
        ui.label(f"Referencia: {_ref_name} · {_fmt_lap(ref_lap.laptime)}").classes(
            "text-sm text-blue-300 bg-blue-950 px-3 py-1 rounded"
        )
        ui.label(f"Tu vuelta: {_drv_name} · {_fmt_lap(drv_lap.laptime)}").classes(
            "text-sm text-purple-300 bg-purple-950 px-3 py-1 rounded"
        )
...
def _render_curvas_tab(rows, trace, charts_of):
    ...
    df["Tiempo ganado/perdido (s)"] = df["Tiempo ganado/perdido (s)"].map(
        lambda v: "%+.3f" % v if v is not None else "—"
    )
    df["Diferencia (km/h)"] = df["Diferencia (km/h)"].map(
        lambda v: "%+.0f" % v if v is not None else "—"
    )
    ...
    corner_sel = ui.select(options=_labels, value=_labels[0], label="Curva a atacar").classes(
        "w-full mb-2"
    )
...
def _render_resumen_tab(charts_of):
    ...
    if not overview and not gg:
        ui.label("No hay graficas de resumen disponibles para esta comparacion.").classes(
            "text-gray-400"
        )


def _render_vuelta_tab(charts_of):
    ui.label("Vista completa de la vuelta — todos los canales").classes(
