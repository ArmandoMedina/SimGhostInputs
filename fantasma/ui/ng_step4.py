...
from .ng_helpers import _POS_LABELS, _FLOWS, _STEPS, _DEFAULT_FLOW, _fmt_lap, _sync_quality_label, start_bg_render
...
async def render(state, navigate):
    ...
    if shutil.which('ffmpeg') is None:
        ...
        _os = _pl.system()
        _install_cmd = (
            '`winget install Gyan.FFmpeg`' if _os == 'Windows'
            else '`brew install ffmpeg`' if _os == 'Darwin'
    ...
    # ── Sincronia ─────────────────────────────────────────────────────────────
    ui.separator().classes('my-4')
    ui.label('② Sincronia: en que segundo del video empieza tu vuelta?').classes('text-sm font-bold text-white mb-1')
    ui.label(
    ...
    if drv_for_sync is None:
        ...
        async def handle_sync_drv(e):
            from .ng_helpers import _save_upload, _load_laps, _best_lap_index
        ...
        ui.label(
            'Usando tu vuelta del Paso 1 (%s) — la que corresponde al video.' % _fmt_lap(drv_for_sync.laptime)
        ).classes('text-xs text-gray-400 mb-2')
    ...
    async def do_autosync():
        ...
        if not _video or not _drv:
            ui.notify('Necesitas el video y la telemetria para la deteccion automatica.', type='warning')
            return
    ...
    with ui.expansion('Sincronizar manualmente (si la deteccion automatica fallo)', icon='tune').classes('w-full mb-4'):
        ui.markdown(
    ...
    with ui.row().classes('gap-8 items-start w-full'):
        with ui.column().classes('gap-3').style('min-width:200px'):
            ...
            offset_input.on('update:model-value', lambda e: state.__setattr__('compose_offset', float(e.value or 0.0)))

        with ui.column().classes('gap-2'):
            preview_img = ui.image('').classes('rounded border border-gray-700').style(
                'max-width:420px;max-height:260px;background:#1e1e1e'
            )
            preview_status = ui.label('Carga el overlay para ver la preview').classes('text-xs text-gray-500')
    ...
    def pick_out_folder():
        ...
        p = _pick_folder('Carpeta de salida', initialdir=out_folder_input.value or _def_out_folder())
        if p:
    ...
    def refresh_summary():
        ...
        if _drv_lap is not None:
            ...
            _clip_line = 'Clip: overlay desde %s min (sin telemetria no se recorta a la vuelta)' % _mss(_offset_val)
    ...
    def _start_compose():
        ...
        with compose_area:
            ...
            cancel_btn = ui.button('Detener composicion', on_click=cancel).props('color=secondary flat')
            ...
            if _job.done:
                ...
                with result_area:
                    if _job.error == '__CANCELLED__':
                        ...
                        ui.label(
                            '✓ Video guardado: %s' % os.path.basename(_out_path)
